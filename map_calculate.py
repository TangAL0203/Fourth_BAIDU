...
def parse_rec(filename):
    ...
    for obj in tree.findall('object'):
        ...
        obj_struct['difficult'] = int(obj.find('difficult').text) # bbox = obj.find('bndbox')
...
def voc_eval(detpath,
    ...
    # read dets
    # such as: comp3_det_test_car.txt:
    ...
    nd = len(image_ids)  #  测试或验证集图片数目
    fp = np.zeros(nd)
    for d in range(nd):
        R = class_recs[image_ids[d]]  # 某张图片，某一类的gt box
...
if __name__ == "__main__":
    ...
    annopath = '../LIP/humanparsing/Annotations/all/{}.xml' # 00001.xml
    imagesetfile = '../LIP/humanparsing/Annotations/LIP_test_id.txt'  # imagename
    classnames = ['hat', 'sunglass', 'upperclothes', 'skirt', 'pants', \
                  'dress', 'belt', 'shoe', 'bag', 'scarf', \
                  'glove', 'coat', 'socks', 'jumpsuits']
    cachedir = './cache'

    f = open('./result/detect_map_result.txt', 'w')
